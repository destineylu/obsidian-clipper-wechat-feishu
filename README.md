...
扩展支持把完整的 Sphinx 说明文档一次保存到 Obsidian，包括所有章节和左侧目录层级。推荐使用按章节保存模式：
...
3. 打开任意 Sphinx 文档页面，例如 `https://docs.datasette.io/en/stable/`。
4. 打开 Web Clipper，等待页面提取完成，点击紫色 **Add to Obsidian** 按钮上方单独的 **收藏整个说明文档** 按钮。
5. 确认标题、页面数量和保存方式。扩展会查找 Sphinx 搜索索引，抓取章节并写入选定的 Vault 和文件夹。

按章节模式会生成每页一篇 Markdown，以及 `00 - Documentation index.md`。目录会尽量保持网站层级：嵌套侧边栏变成嵌套 Wiki 链接，子页面进入父页面文件夹，目录分组也会保留；侧边栏没有列出的页面归入 `Other pages`，无法解析侧边栏时回退使用 Sphinx 的 `docname` 路径。重复执行会覆盖相同路径的笔记，但不会删除其他笔记。

如果配套插件不可用、未配对、连接到其他 Vault 或版本过旧，确认框会明确显示“合并为一篇 Markdown”模式。此时整套文档按目录顺序合并为一篇笔记，页面标题和页面内部标题会自动形成层级。

如果看不到“收藏整个说明文档”按钮，请刷新当前文档页面并重新打开扩展；该按钮只会在识别到 Sphinx 文档时显示，PDF、浏览器内部页面和普通网页不支持此功能。
